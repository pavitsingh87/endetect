		...
		<div id="pen2-left">
			<div ng-controller="lic_customersCrtl" >
				<div class="message-container" style="width:680px;">
					<h2 style="width:450px;float:left;line-height:5px;">Pending License Activation</h2>
    		...
    		<div class="row">
		        ...
		        <div class="col-md-3">
					...
					<input name="Search" type="submit" id="searchButton1"  style="right:16px;">
		        </div>
		        <br style="clear:both">
		    </div>
    <br/>
				...
				<tbody>
					<tr ng-repeat="data in filtered = (list | orderBy : predicate :reverse) | limitTo:entryLimit">
					   ...
					   <td style="font-size:11px;"><span style='float:left' ng-bind="data.serialnum"></span></td>
					    ...
					    <td style="font-size:11px;"><span ng-show="data.selectedItem.sno!=0" ng-bind="data.selectedItem.name"></span> <a  ng-show="data.selectedItem.name && data.selectedItem.sno!=0 " href="javascript:void(0)" ng-click="removeCurrentRelease(data.selectedItem, $index)"><i class="glyphicon glyphicon-pencil"></i></a> <select ng-show="!data.selectedItem.name || data.selectedItem.sno==0" ng-model="data.selectedItem" ng-init='data.selectedItem={"sno":"0","name":"New User","selected":0}' ng-options="item as item.name for item in releasearr | filter: {selected:0} track by item.sno" ng-change="updateList(data.selectedItem, $index)"></select></td>
					</tr>
				</tbody>
            </table>
