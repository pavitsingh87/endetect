...
if(isset($_REQUEST['ddatasearch']))
{
	...
	$decdatastr ='"must" : { "query_string": { "default_operator" : "AND", "analyze_wildcard": true, 	"query" : "*'.$ddatasearch.'*" } }, ';
}
...
if(isset($_SESSION['ownerid']) || (strlen($_SESSION['ownerid'])>0))
{
		if(isset($results))
		{
			foreach($results as $r)
			{
				...
				$vtext = $json_user;
				$dtypeq = $conn->query("select originaltxt from searchin where id='".$r['_source']['dtype']."'");
				$dtypequery = @$dtypeq->fetch_assoc();
				$queryi =$conn->query("select lastaccesstime,profilepic,name,designation,dept,groupid from U_endusers where sno='".$r['_source']['enduserid']."'");
				...
				?>
					...
					<div class="dropdown" style="float:right;padding:5px;">
				      ...
				      <div id="note<?php echo $r['_source']['sno']?>" style="float:right;margin-right:5px;cursor:pointer;">
						<?php if($noteflag=='1') { ?>
						<!-- <span class="glyphicon glyphicon-list-alt"  onclick="shownote('<?php echo $r['_source']['sno']?>')"></span> -->
						<img src="images/notes.png" alt="" onclick="shownote('<?php echo $r['_source']['sno']?>')">
						<?php } ?>
					  </div>
						...
						<div id="stbody6" class="stbody">
							...
							<div class="stimg">
								<div class="divboximage">
									<a href="userprofile.php?eid=<?php echo base64_encode($r['_source']['enduserid']);?>">
										<img src="<?php echo $queryimage['profilepic'];?>" class="home_profile_pic">
										<?php // echo ago2(strtotime($queryimage['lastaccesstime'])); ?>
									</a>
								</div>
							</div>
							<!-- post img end -->
							...
							?>
								...
								<b>
									<div class="divboxname" style="float:left">
										...
										<div class="prevnextsearch glyphicon glyphicon-search" onclick="showprevnext('<?php echo $r['_source']['sno']; ?>','<?php echo $r['_source']['enduserid']?>')" id="mvfs<?php echo $r['_source']['sno']; ?>"></div>
										&nbsp;&nbsp;&nbsp;
									</div>
								...
								<div id="stexpandbox<?php echo $r['_source']['sno'];?>">
									...
									<div id="stexpand6">
										...
										<img  style="cursor:pointer;border:1px solid #ccc;padding:3px;" src="thumbnail.php?src=<?php echo $imageDataEncoded; ?>&w=200&h=113"  onclick="popupgallery('popUpDivgallery','<?php echo $watchlist; ?>','<?php echo $imageDataEncoded; ?>','<?php echo $dtype; ?>','<?php echo mysqli_escape_string($r['_source']['app_title']); ?>','<?php echo $sno;?>','<?php echo $enduserid;?>','<b><?php echo $queryimage['name'];?></b>','<b><?php echo $queryimage['designation']; ?></b> (<?php echo  ago1($datefind); ?>)')"/></div>
										<?php
										 }
										 else { ?>
										<div id="divexpandbox<?php echo $r['_source']['sno']; ?>"  style="margin-top:10px;" class="<?php echo $decdatastyle;?>"><p><xmp style="<?php echo $xmpstyle; ?>"><?php echo $vtext;?></xmp></p></div>
										<?php } ?>
										</div>
		...
		<div class="panel-heading" onclick="loadmore(this)" id="loadmore<?php echo $r['_source']['sno'] ?>">
			<a ng-hide="ascrec">
